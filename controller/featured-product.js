...
exports.getAllFeaturedProduct = async (req, res, next) => {
    try {
        ...
        let queryData;
        if (queryField) {
            queryData = FeaturedProduct.find({type: queryField});
        } else  {
        ...
        const data = await queryData.populate(            
            {
                path: 'product',
                select: '_id productName productSlug sku images price discountType discountAmount quantity productVisibility category categorySlug',
...
exports.getSingleFeaturedProductById = async (req, res, next) => {
    ...
    try {
        const data = await FeaturedProduct.findOne(query)
...
exports.editFeaturedProduct = (req, res, next) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        const error = new Error('Input Validation Error! Please complete required information.');
        error.statusCode = 422;
    ...
    FeaturedProduct.findOneAndUpdate(query, push)
        .then(() => {
            res.status(200).json({
