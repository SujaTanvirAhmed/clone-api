...
exports.insertManyWarranty = async (req, res, next) => {
    ...
    try {
        ...
        data.forEach(async (f) => {
            // console.log('f',f)
            if (!f.warrantyPeriod) {
                f.warrantyPeriod = 'N/A'
            }
            const hasData = warranties.find(g => g.invoiceNumber === f.invoiceNumber);
            if (hasData) {
                await Warranty.findOneAndUpdate({invoiceNumber: f.invoiceNumber}, {
                    $set: f
                })
            } else {
                const warranty = new Warranty(f);
                await warranty.save();
            }
        })
        ...
        res.status(200).json({
            message: `${data && data.length ? data.length : 0} Warranty\'s imported Successfully!`
        });
    } catch (err) {
        console.log(err)
        if (!err.statusCode) {
