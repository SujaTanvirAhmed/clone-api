...
exports.getPromoPagebyId = async (req, res, next) => {
    const errors = validationResult(req);
    ...
    const promoPageId = req.params.promoPageId;
    const promopage = await PromoPage.findOne({_id: promoPageId});
...
exports.editPromoPageInfo = async (req, res, next) => {

    const updatedData = req.body;

    try {
        await PromoPage.updateOne({_id: updatedData._id}, {$set: updatedData});
        res.status(200).json({
            message: 'Promo Page Updated Successfully!'
        });
